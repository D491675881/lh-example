...
public class HttpClientUtilTest {
    ...
    public static void main(String[] args) throws HttpProcessException, FileNotFoundException {
        ...
        //HttpClientUtilTest.down(config);					//下载，需要调用config.out(fileOutputStream对象)
        //HttpClientUtilTest.upload(config);				//上传，需要调用config.files(文件路径数组)

        //如果指向看是否访问正常
        //String result3 = HttpClientUtilTest.head(config); // 返回Http协议号+状态码
        //int statusCode = HttpClientUtilTest.status(config);//返回状态码
